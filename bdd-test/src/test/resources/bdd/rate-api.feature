...
Feature: Rate Service
	...
	Scenario Outline: Fetch the latest rate with base currency '<baseCurrency>'
		Given forex rate is available for base currency '<baseCurrency>'
	...
	Scenario Outline: Make a rate booking
		Given forex rate is available for base currency '<baseCurrency>' and counter currency '<counterCurrency>'
		And Customer saved in database:
			| id 			| name 		| tier 	|
			| <custmerId>	| John Doe	| 10	|
